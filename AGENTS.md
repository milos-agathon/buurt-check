...
**Stage: F1-F5 implemented. Full "Clear Signal Hybrid" design system applied. 0-100 risk scoring. Tab navigation. Shortlist + compare built. PDF export pending.**

### What exists
- `backend/` — FastAPI app with 9 endpoints: address suggest/lookup, building facts, 3D building/neighborhood, risk cards (with 0-100 scores + severity), neighborhood stats, WMS tile proxy, viewing questions. BAG lookups use OGC XML Filter. 3DBAG uses tiled fetch (direct target + grid tiles). LoD 2.2 roof geometry parsing (feature-flagged). Risk scoring normalizes noise/air/climate/sunlight to 0-100. CBS integration with buurt-code + bbox fallback. Redis cache with circuit breaker. 242 passing tests + live smoke tests.
- `frontend/` — Vite + React + TypeScript with "Clear Signal Hybrid" design system. Satoshi font, Electric Teal accent, CSS design tokens. 3-tab navigation (Search, Briefing, Saved) with frosted glass tab bar. Loading screen with building animation. Dossier screen: address header, summary strip, 3D viewer, 2x2 risk tiles with 0-100 scores, risk detail views with comparison charts + viewing questions, neighborhood stats, viewing checklist, action bar. Shortlist (max 3) with compare screen. 295 passing Vitest tests. i18n with ~200 keys per language.
...
### What's next
- Maintain quality gates: `ruff check`, backend pytest (242+, excluding live), frontend vitest (295+), `npm run build`.
- Implement PDF export (Quick Brief 1-page + Full Dossier 3-4 pages).
...
4. **Test count baselines (updated 2026-02-09).** Backend: 242 non-live tests (+ live smoke tests). Frontend: 295 tests. Any change must maintain or increase these numbers.
...
- **Backend: 242 non-live + live smoke tests.** Previous baseline: 162.
- **Frontend: 295 tests.** Previous baseline: 150. Major additions from design system components, navigation, shortlist/compare, settings, and integration tests.
