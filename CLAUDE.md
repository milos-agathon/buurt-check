...
**Stage: F1-F6 fully implemented + Phase 1B shipped (Leefbaarometer livability, SoilInfoCard, lead pipe proxy). All MVP features shipped + Phase 1A mobile UI premium merged. "Polar Frost" design system with full palette.md token alignment. 0-100 risk scoring. Tab navigation with spring-animated pill. Shortlist + compare with parallel coordinates. PDF export (Quick Brief + Full Dossier) with Polar Frost branding. Tier B signals (energy label + crime). Property warnings (foundation, erfpacht, VvE, asbestos, lead pipe). LivabilityCard + LivabilityDetailView with trend chart and comparison bars. SoilInfoCard with Bodemloket link and lead pipe warning. AttentionSummary badge (risk + property warnings + lead pipe). Risk comparisons (city/NL/WHO baselines). Dark mode (OLED-ready). Recent searches. Simplified 3D viewer (static context card, no interactive controls). Building orientation estimation. Framer Motion gestures (DossierSheet, shared element transitions, press states, haptic feedback). Skeleton loading. Accessibility + visual regression tests. Bundle optimization (React.lazy + vendor chunks). Post-implementation audit hardening (6 rounds). Mapillary removed from scope.**

### What exists
- `backend/` — FastAPI app with 15 endpoints: address suggest/lookup, building facts, 3D building/neighborhood, risk cards (with 0-100 scores + severity), risk comparisons, neighborhood stats, WMS tile proxy, viewing questions, PDF export (quick_brief + full_dossier), tier-b signals, property warnings (with lead pipe), livability. BAG lookups use OGC XML Filter. 3DBAG uses tiled fetch (direct target + grid tiles). LoD 2.2 roof geometry parsing (feature-flagged). Risk scoring normalizes noise/air/climate/sunlight to 0-100. Risk comparisons with urbanization-stratified baselines. CBS integration with buurt-code + bbox fallback. Tier B: EP-Online energy labels + CBS OData crime stats. Property warnings: foundation risk (BRO soil + subsidence), erfpacht (municipality config), VvE fund, asbestos detection, lead pipe proxy (BAG year < 1960). Leefbaarometer: livability score + 5 dimensions + 9-point historical trend + wijk/gemeente comparison. Redis cache with circuit breaker. fpdf2 for PDF generation with Satoshi TTF. Building orientation estimation from footprint geometry. 405 non-live passing tests + live smoke tests. Cache version: v13.
- `frontend/` — Vite + React + TypeScript + Framer Motion with "Polar Frost" design system. Satoshi font, Arctic Teal accent, ~195 CSS design tokens (including slate/teal scales, nav/overlay utility, choropleth ramps). 3-tab navigation (Search, Briefing, Saved) with spring-animated teal pill. Skeleton loading cards (replaces full-screen blocker). DossierSheet with gesture drag + 4 snap points (collapsed/default/expanded/fullscreen). Dossier screen (canonical 14-section order enforced by regression test): AttentionSummary, AddressHeader, SummaryStrip, BuildingFacts, RiskTiles, PropertyWarnings, SoilInfo, Livability, 3DViewer, Sunlight, NeighborhoodStats, TierB, ViewingChecklist, ActionBar. LivabilityCard with dimension bars + trend chart + comparison. LivabilityDetailView with full drilldown. SoilInfoCard with Bodemloket link + collapsible viewing questions + lead pipe warning. Press states via usePressable hook. Haptic feedback on bookmark/tab/export. Dark mode (light/dark/system, OLED-ready #000000 base). Recent searches. Shortlist (max 3) with compare screen + parallel coordinates chart. Settings screen. PDOK BRT grijs basemap (CSS filter invert for dark mode). Bundle: React.lazy for 4 heavy components, vendor chunks (react/map/three/framer-motion). 421 passing Vitest tests + accessibility tests + keyboard nav tests + performance budget tests + i18n floor assertions. 10 visual regression snapshots (5 screens x 2 themes). 1 Playwright E2E test (dossier section order). i18n with ~380 keys per language (parity + floor enforced).
...
### What's next
- **Phase 1A merged + Phase 1B shipped.** `feat/mobile-ui-premium` merged to `main` on 2026-02-15 (commit `54c6686`). Phase 1B (Leefbaarometer, SoilInfoCard, lead pipe proxy) implemented and hardened through 6 audit rounds.
- Maintain quality gates: `ruff check`, backend pytest (405+, excluding live), frontend vitest (421+), `npm run build`.
...
4. **Leefbaarometer is production-ready.** WFS 2.0.0 with CQL_FILTER support, GeoJSON output, 5 livability dimensions on a 1-9 scale. No auth, no rate limits. Straightforward integration following existing WFS patterns in `risk_cards.py`.
...
## Learnings from Feb 15 sessions (Phase 1B implementation + post-audit hardening)

### Phase 1B Planning (5 sessions, 7 plan revisions, 23 issues resolved)

1. **Plan revision cycles are valuable but expensive.** The Phase 1B plan went through 7 revisions (v1→v7) across 5 sessions. Each cycle found 2-6 material issues (undefined types, contradictory prop designs, overlapping cache keys, duplicate UI elements, stale approval gates). Single-pass plan approval would have caused implementation bugs — but 7 iterations suggests insufficient upfront rigor. A structured pre-submission checklist (types defined? props match usage? counts consistent? design spec reconciled?) could reduce iterations.
2. **Plans that claim to be "corrected" can still have material issues.** Never trust a self-described "corrected" plan without independent review. Issue counts across versions: v3(6), v4(6), v5(5), v6(3), v7(3).
3. **Design Revision sections formalize spec deviations.** When implementation deviates from design spec (e.g., horizontal bars instead of radar chart, endpoint path change), document as a formal table: original requirement, revised requirement, rationale. Prevents future confusion.
4. **Subagent codebase exploration before plan writing is essential.** Two parallel subagents confirmed actual codebase state (main branch had no Phase 1A code) in ~2 minutes, preventing planning based on wrong assumptions.
5. **Execution-readiness requires zero blocking gates.** A plan that says "pending stakeholder approval" at any step is NOT execution-ready. Either finalize the decision or explicitly scope around the uncertainty.
6. **Issue numbering across plan versions provides traceability.** Running counter #1 through #23 across v1-v7, referenced in resolution text, decision log, acceptance criteria, and changelog, prevents the same issue from being "fixed" multiple times in different ways.

### Phase 1B Execution (1 session, 12 commits, 36 minutes)

7. **Detailed plan enables zero-intervention execution.** A 17-step plan with exact file paths, function signatures, cache key formats, and test count targets completed in 36 minutes with zero user interventions and 12 clean commits. The plan-first, execute-second pattern works cleanly when the plan is sufficiently detailed.
8. **Quality gates at both post-merge AND final steps.** Running quality gates immediately after merging `feat/mobile-ui-premium` (Step 0) caught a Pydantic mutable-default bug before it could compound through 16 subsequent implementation steps. Don't skip early-stage quality gates.
9. **Pydantic v2 mutable defaults: use `Field(default_factory=list)`.** Using bare `list` as default (`messages: list[str] = []`) creates a mutable default shared across instances. This bug survived feature branch review and was caught during post-merge quality gates. Pattern: always search for `: list = []` and `: dict = {}` in Pydantic models after merges.
10. **Leefbaarometer integration is fast.** Point queries complete in <100ms. 11 parallel WFS queries (1 current + 9 historical + 2 comparison) via `asyncio.gather` complete in <500ms total. Cache key: `livability_full:{rd_x:.0f}:{rd_y:.0f}`, TTL 30 days.
11. **Score normalization formula.** Leefbaarometer 1-9 scale → 0-100: `(raw_score - 1) / 8 * 100`.
12. **Endpoint-only caching for multi-query aggregation.** Service functions are pure (no caching). Caching happens exclusively at the endpoint after assembling the full response. Prevents key-shape ambiguity from two caching levels writing different data shapes to overlapping keys.
13. **API contract for "no data": HTTP 200 + `{available: false}`.** Backend returns 200 for valid `vbo_id` even when data is unavailable. Frontend checks `data.available === false` and returns `null` to components. This is a valid business response, not an error.

### Post-Implementation Audit Cycle (6 sessions, iterative audit-fix-reaudit)

14. **Plan-compliance auditing catches drift that passing tests miss.** With 418 passing tests and a clean build, 6 behavioral divergences from the v7 spec went undetected. Tests verify code correctness against assertions, not against the spec. Periodic plan-compliance audits are a separate quality gate.
15. **Tests that lock in wrong behavior are worse than absent tests.** Three AttentionSummary tests asserted that livability < 40 triggers a flag — behavior the v7 spec prohibited. These tests created false confidence. When updating a spec, update tests to match the spec.
16. **Stale assessment feedback: verify all findings against HEAD before implementing.** This is the third occurrence of stale assessments causing false positives (Feb 10: 89% error rate, Feb 15: 100% false positive rate). Rule: never act on assessment findings without first verifying each finding against current HEAD.
17. **Component props should be minimal.** Pass only the fields a component needs, not entire response objects. SoilInfoCard was refactored from receiving `PropertyWarningsResponse` to just `{ leadPipeFlagged?: boolean; constructionYear?: number }`.
18. **Dossier section order needs automated enforcement.** With 13+ sections, manual verification is unreliable. Regression test checking `compareDocumentPosition` catches reordering bugs. All section root elements need `data-testid` attributes proactively.
19. **`boundingBox().y` is fragile for layout order E2E tests.** Breaks with `position: fixed` or `position: sticky` elements (ActionBar renders at viewport bottom regardless of DOM position). Use `compareDocumentPosition` for DOM tree order — immune to all CSS positioning modes.
20. **Vite dev proxy must default to `127.0.0.1`.** LAN IP auto-detection picks a non-localhost interface, causing ECONNREFUSED when Playwright starts the backend on 127.0.0.1. Use env var `VITE_DEV_API_PROXY_TARGET` for LAN development scenarios.
21. **Cache keys must include ALL parameters affecting the response.** Property warnings cache key v2: `property_warnings:v2:{vbo_id}:{rd_x}:{rd_y}:{cy}:{nu}:{mu}`. Audit cache key composition against the full parameter list as a standard review step.
22. **Municipality normalization must be consistent across business logic AND cache keys.** Use `.strip().casefold()` on both input and comparison targets. Without this, `"Amsterdam"` matches but `"amsterdam"` doesn't — while both share the same normalized cache key, causing cache poisoning.
23. **Cache test patterns.** Cache hit tests need call-count assertions (proving upstream NOT called on second request). No-cache-on-empty tests need a two-call pattern: call once (get empty), call again, assert upstream called both times AND cache remains empty.
24. **Cache key regression tests need variation + normalization.** Variation: different inputs produce different keys (no false sharing). Normalization: equivalent inputs produce the same key (no cache fragmentation).
25. **i18n tests need both parity AND floor assertions.** Parity between two empty files passes parity. Add minimum key count assertion (`>= 380`).
26. **Native `<details>/<summary>` for collapsible static content.** Zero JS, accessible, semantic. Preferred over `useState` toggle for non-interactive content sections like viewing questions.
27. **Single-ownership principle for UI signals.** Each data signal (like lead pipe risk) should be owned by exactly one UI card for rendering. Duplicating warnings across multiple cards over-weights signals. Backend model can still expose the field for cross-cutting concerns (AttentionSummary badge counting).
28. **Sequential dependency chaining in parallel IIFEs.** Property warnings depend on building facts (needs constructionYear). Chain them in one IIFE (building facts → property warnings), while the IIFE itself runs in parallel with independent fetches (risks, stats, livability).

### Updated test baselines (2026-02-15 post-Phase 1B + hardening)

- **Backend: 405 non-live tests** (was 381). Main: 402 non-live at last gate + 3 regression tests.
- **Frontend: 421 tests** (was 385). i18n: 380 keys per language with floor enforcement.
- **E2E: 1 Playwright test** (dossier section order via `compareDocumentPosition`). Runs separately from Vitest.
