...
## Session Learnings (2026-07-04) -- Nightly Compound Session-Limit Failure

Findings from the 2026-07-04 nightly compound run (health check of the 2026-07-03 run):

- **A second silent-failure mode for the nightly compound cron: session-limit exhaustion.** One night after the 30-night auth outage was fixed (2026-07-02), the 2026-07-03 23:30 run died on its first turn with "You've hit your session limit -- resets 12:40am (Europe/Amsterdam)". The 23:30 schedule sits just before a limit-reset window, so a day of heavy interactive usage starves the cron. Mitigations: schedule the run after the reset (~00:45) or have the run detect the limit message and reschedule itself past the reset time.
- **Failure stubs are indistinguishable from healthy transcripts by file size.** The 2026-07-03 stub was 49 KB (11 messages) vs 48.7 KB for the June auth stubs vs 70 KB for a real run start -- size and mtime look normal either way. The health check must read transcript content (grep for limit/auth error text in the last assistant message), not just confirm the file exists.
- **The `compound-engineering` skill referenced by the nightly cron prompt does not exist in this environment.** `Skill(compound-engineering)` returns "Unknown skill" and no skill file exists under `.claude/` (project or user). Every run must fall back to the manual process: health-check prior runs -> read last-24h transcripts -> extract learnings -> update CLAUDE.md -> commit -> push. Either install the skill or reword the cron prompt to describe the manual process directly.
- **No learnings were lost this time**: no dev sessions occurred between the Jul 2 compound and the Jul 3 failure, so the one-night gap had nothing to compound. The June outage was only costly because 30 nights coincided with expiring transcripts.
