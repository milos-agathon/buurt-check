...
* The mobile sticky nav now keeps brand and language controls in the first row and moves `How it works`, `Pricing`, `FAQ`, and the app CTA into a single horizontally scrollable no-wrap row below `720px`
...
* `npm run landing:test:e2e` passes locally across mobile and desktop projects (`21 passed, 1 skipped` on the current workspace)
* `npm run landing:perf:live` shows no material LCP regression versus the current deployed landing page, so EP5 is closed for the current landing scope
