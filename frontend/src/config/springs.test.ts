...
import { SPRING_EXPAND, SPRING_REVEAL, SPRING_TAB } from './springs';
...
describe('Spring configurations', () => {
  ...
  it('all springs have type, stiffness, and damping', () => {
    for (const spring of [SPRING_EXPAND, SPRING_REVEAL, SPRING_TAB]) {
  ...
  it('SPRING_TAB is snappiest (highest stiffness)', () => {
    ...
    expect(SPRING_TAB.stiffness).toBeGreaterThanOrEqual(SPRING_REVEAL.stiffness);
  });
});
