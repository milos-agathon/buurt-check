...
const BLOCKED_PREBID_ANALYTICS_KEY_PATTERNS = [
  ...BLOCKED_ANALYTICS_KEY_PATTERNS,
  /(^|_)(share|token|email|contact|hash)(_.*)?$/i,
  /(^|_)(source|evidence|finding|question|request|recipient|limitation|note|text|copy)(_.*)?$/i,
  /(url|href|link)$/i,
];
...
function sanitizeAnalyticsProperties(
  ...
  blockedPatterns = BLOCKED_ANALYTICS_KEY_PATTERNS,
  ...
  const sanitizedEntries = Object.entries(properties)
    .filter(([key]) => !blockedPatterns.some((pattern) => pattern.test(key)))
    .map(([key, value]) => {
...
export function sanitizePrebidAnalyticsProperties(
  properties?: Record<string, AnalyticsProperty>,
): Record<string, AnalyticsProperty> | undefined {
  return sanitizeAnalyticsProperties(properties, BLOCKED_PREBID_ANALYTICS_KEY_PATTERNS);
}

export function trackPrebidEvent(
  name: string,
  properties?: Record<string, AnalyticsProperty>,
): void {
  const sanitizedProperties = sanitizePrebidAnalyticsProperties(properties);
  trackEvent(`prebid_${name}`, sanitizedProperties);
}
