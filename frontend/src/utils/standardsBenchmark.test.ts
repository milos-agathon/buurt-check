...
import { getEN17037Level, getWinterExposureRatioLevel } from './standardsBenchmark';
...
describe('getWinterExposureRatioLevel', () => {
  it('returns "high" for >= 80% of possible sun hours', () => {
    expect(getWinterExposureRatioLevel(7.0, 7.5)).toBe('high');
  });

  it('returns "high" at exact 80% boundary', () => {
    expect(getWinterExposureRatioLevel(6.0, 7.5)).toBe('high');
  });

  it('returns "medium" for >= 50% of possible sun hours', () => {
    expect(getWinterExposureRatioLevel(4.0, 7.5)).toBe('medium');
  });

  it('returns "medium" at exact 50% boundary', () => {
    expect(getWinterExposureRatioLevel(3.75, 7.5)).toBe('medium');
  });

  it('returns "medium" just below 80% boundary', () => {
    expect(getWinterExposureRatioLevel(5.99, 7.5)).toBe('medium');
  });

  it('returns "below" for < 50% of possible sun hours', () => {
    expect(getWinterExposureRatioLevel(2.0, 7.5)).toBe('below');
  });

  it('returns "below" just under 50% boundary', () => {
    expect(getWinterExposureRatioLevel(3.74, 7.5)).toBe('below');
  });

  it('returns "below" when possibleHours is 0', () => {
    expect(getWinterExposureRatioLevel(5.0, 0)).toBe('below');
  });

  it('returns "below" when possibleHours is negative', () => {
    expect(getWinterExposureRatioLevel(5.0, -1)).toBe('below');
  });
});
